...
import { signalRmarket } from "../../../redux/Actions/marketActions";
...
export default function CoinCardComponent(props) {
  ...
  const price = parseFloat(props.ticker.lastPrice);
  ...
  const CoinDetail = () => {
    dispatch(signalRDisconnect()).then(() => {
      dispatch(signalRmarket(userToken, props.ticker.symbol)).then(() => {
